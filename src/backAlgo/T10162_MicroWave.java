...
class MicroWave {
	//5분버튼을 처리하는 메서드
	private static int[] push_FiveMin(int num) {
		if(num < 300) {
			int[] arr = {0, num};
			return arr;
		}
		else {
			int[] arr = {num/300 , num%300};
			return arr;
		}
	}
	//1분버튼을 처리하는 메서드
	private static int[] push_OneMin(int[] arr) {
		if(arr[1] < 60) {
			int[] arr2 = {0, arr[1]};
			return arr2;
		}
		else {
			int[] arr2 = {arr[1]/60 , arr[1]%60};
			return arr2;
		}
	}
	//10초버튼을 처리하는 메서드
	private static int[] push_TenSec(int[] arr) {
		if(arr[1] < 10) {
			int[] arr3 = {0, arr[1]};
			return arr3;
		}
		else{
			int[] arr3 = {arr[1]/10 , arr[1]%10};
			return arr3;
		}
	}
	...
	public static void main(String[] args) {
		...
		int[] arr5min = push_FiveMin(target);
		int[] arr1min = push_OneMin(arr5min);
		int[] arr10sec = push_TenSec(arr1min);
		
		if(arr10sec[1] == 0) System.out.printf("%d %d %d" , arr5min[0], arr1min[0], arr10sec[0]);
