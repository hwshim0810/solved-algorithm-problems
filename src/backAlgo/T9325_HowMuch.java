...
자동차에 여러가지 옵션을 포함시킬 수 있는데 해빈이는 덧셈과 곱셈을 하지 못하기 때문에 친구 태완이에게 도움을 청했다. 
...
@see https://www.acmicpc.net/problem/9325
 */
...
class HowMuch {
	...
	public static void main(String[] args) {
 		try(Scanner scan = new Scanner(System.in)) {
 			...
 			for(int i = 0; i < rpt; ++i) {
 				int car_val = scan.nextInt();
 				int opt_num = scan.nextInt();
 				
 				for(int j = 0; j < opt_num; ++j) {
 					car_val += (scan.nextInt() * scan.nextInt());
 				}
 				System.out.println(car_val);
 			}
 		}
	}
