...
class ScoreCounting {
	...
	public static void main(String[] args) {
		try(Scanner scan = new Scanner(System.in)) {
			int sum = 0; // 출력 할 점수의 합
			int plus = 0; // 더할 점수 선언
			int rpt = scan.nextInt(); // 반복횟수
			int[] arr = new int[rpt]; // 점수 배열
			
			for(int i = 0; i < rpt; ++i) arr[i] = scan.nextInt(); // 점수배열에 점수저장
			
			for(int x : arr) {
				if(x == 1) plus++; // 맞은경우는 점수를 1점올려줌
				else plus = 0; // 틀린경우 0으로 초기화
				sum += plus; // 점수를 더해줌
			}
