...
class ATM {
	...
	private static int minVal(List<Integer> list, int idx) {
		if(idx < 0) return 0;
		
		int sum = 0;
		
		for(int i = 0; i < idx; i++) sum += list.get(i);
		
		return sum + minVal(list, idx-1);
	}
	...
	public static void main(String[] args) {
		try(Scanner scan = new Scanner(System.in)) {
			...
			for(int i = 0; i < size; i++) list.add(scan.nextInt());
			
			Collections.sort(list, Comparator.reverseOrder());
			
			System.out.println(minVal(list, size-1));
		}
	}
